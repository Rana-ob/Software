...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_match'])) {
    ...
    $sql = "DELETE FROM Match WHERE MATCH_ID = ?";
...
$sql = "SELECT MATCH_ID, TEAM1, TEAM2 FROM Match";
...
<head>
    ...
    <style>
        body { font-family: 'Almarai', sans-serif; background-color: #0a0213; color: white; text-align: center; }
        .container { width: 50%; margin: auto; margin-top: 50px; }
        .card { background-color: #1a0125; padding: 20px; border-radius: 10px; box-shadow: 0 0 10px rgba(255, 255, 255, 0.2); text-align: center; margin-bottom: 15px; }
        .delete-btn { background-color: red; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer; margin-top: 10px; }
        .header {
           display: flex;
           align-items: center;
           height: 80px;
